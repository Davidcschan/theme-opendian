...
1. **只改 `theme.css`，且只在本目录（`Opendian`）。** 这是唯一的 `main` 分支工作区。
   - **单分支架构（自 v1.13.0 起）**：不再有 dev 分支，所有开发都在 main 上。
...
DEV_VAULT 已通过 symlink 把本主题挂进 `../DEV_VAULT/.obsidian/themes/Opendian`，
改完 `theme.css` 后：
...
- light + dark 都验过。
- 没有顺手改到无关区段。
- **如果要发版**：bump version 后打 tag（无 v 前缀，== version）并 `git push origin main --tags`，workflow 自动接管（见「发版流程」）。
...
4. **tag 指向的 commit 的 manifest `name` 必须是 `"Opendian"`**、必填字段齐。
   > ⚠️ 这是 v1.12.5 被拒的血泪：Obsidian 审核读的是 **tag 关联 commit** 的 manifest，
   > 不是 release 资产。单分支架构下 tag 永远打在 main，manifest name 永远是 `Opendian`，
   > 所以这条天然满足——但**绝不能**在仓库里出现第二个带 `Opendian-dev` 的分支。
...
> 自 v1.13.0 起采用**单分支架构**：所有开发在 main 上，不再有 dev 分支。
> 发版由 `.github/workflows/release.yml` 自动化——校验合规、打包白名单资产、发 release、自检。
> 红线（无 v、tag==version、name==Opendian、资产齐、`--latest`、非 draft、无外部资源）
...
# 2. bump manifest.json 的 version，例如 1.13.1
# 3. commit + 打 tag（tag 必须无 v 前缀，必须 == version）+ 一条命令推送：
git add -A
git commit -m "v1.13.1: <改动摘要>"
git tag 1.13.1
git push origin main --tags
# 完成。workflow 接管，约 1 分钟后 release 上线。
```

**就这些。** 因为 tag 打在 main commit 上，而 main 的 manifest name 永远是 `Opendian`，
所以 Obsidian 审核读 tag 关联 commit 时永远看到正确的 name——这是单分支架构根治
v1.12.5 审核拒绝问题的关键。

### workflow 做了什么（无需你管，但要知道）

1. **校验红线**：tag 无 `v` 前缀、tag == manifest version、**manifest name == "Opendian"**、
...
（`Opendian-dev` vs `Opendian`），但 Obsidian 审核读 tag 关联 commit 的 manifest——
tag 打在 dev 上时 name 是 `Opendian-dev`，审核直接拒（v1.12.5 的血泪）。
单分支消灭了这个矛盾：只有一个 name `Opendian`，tag 永远在 main。
...
```bash
# 应急流程（在 main 上）：
# 1. 确保 manifest version 已 bump、name 是 Opendian
